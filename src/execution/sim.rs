...
use crate::execution::client::{BuyIntent, OrderClient, PlaceOrderRequest, PlaceOrderResult};
use crate::position::{ManagedOrder, OrderStatus};
use crate::tui::app::{AppState, BookLevel, ChaseSide};

/// Deterministic dry-run execution model. Strategy emits BuyIntents with the same target / qty /
/// reason; this simulator owns realism: submit latency, maker queue position, FAK taker fills,
/// cancel latency, TTL + reprice cancels.
#[derive(Clone, Debug)]
...
impl ExecutionSim {
    ...
    pub fn submit_buy_intent(&self, s: &mut AppState, intent: &BuyIntent, now_ms: i64) {
        if pending_order(s, intent.side).is_some() {
            return;
        }
        ...
        let mut order =
            ManagedOrder::new_buy(intent.side, intent.target, intent.qty, now_ms, intent.reason);
...
fn take_pending_order(s: &mut AppState, side: ChaseSide) -> Option<ManagedOrder> {
    match side {
        ChaseSide::Up => s.pending_order_up.take(),
        ChaseSide::Down => s.pending_order_down.take(),
    }
}

fn set_pending_order(s: &mut AppState, side: ChaseSide, order: Option<ManagedOrder>) {
    match side {
        ChaseSide::Up => s.pending_order_up = order,
        ChaseSide::Down => s.pending_order_down = order,
    }
}

fn pending_order(s: &AppState, side: ChaseSide) -> Option<&ManagedOrder> {
    match side {
        ChaseSide::Up => s.pending_order_up.as_ref(),
        ChaseSide::Down => s.pending_order_down.as_ref(),
    }
}
...
fn asks_for_side(s: &AppState, side: ChaseSide) -> &[BookLevel] {
    match side {
        ChaseSide::Up => &s.poly_asks,
        ChaseSide::Down => &s.poly_down_asks,
    }
}

fn bids_for_side(s: &AppState, side: ChaseSide) -> &[BookLevel] {
    match side {
        ChaseSide::Up => &s.poly_bids,
        ChaseSide::Down => &s.poly_down_bids,
    }
}

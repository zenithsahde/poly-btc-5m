...
pub struct PositionView {
    ...
    pub ioc_jump_chase_orders: usize,
    pub ioc_jump_chase_filled: usize,
    pub ioc_jump_chase_fill_rate: f64,
...
pub struct OrderEventView {
    ...
    pub side: String,
    /// 触发原因: chase / rebal / jump_chase
    #[serde(default)]
    pub reason: String,
    /// 人话故事："chase 追 UP 0.42 → 成交 100张 花费 $42"
    pub summary: String,
    pub qty: f64,
    pub price: f64,
    /// 本笔事件的现金流（成交=花费的 USDC 负号；拒单/挂单=0）
    pub pnl: f64,
}
...
pub struct FairValueView {
    ...
    /// Chainlink BTC/USD spot price (结算源)；None 表示 WS 未连或未启用
    pub chainlink_price: Option<f64>,
    /// binance_mid - chainlink_price；正=binance 高
    pub basis_vs_chainlink: Option<f64>,
    /// 1s 内 binance mid 累计变化（带符号），±$30 触发 JumpChase
    pub binance_jump_1s: f64,
    /// 距上次 JumpChase 触发的毫秒；None=无近期 jump
    pub last_jump_age_ms: Option<i64>,
    /// 上次 jump 方向: +1 上跳 / -1 下跳

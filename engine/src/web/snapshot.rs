...
pub fn build(state: &Arc<RwLock<AppState>>) -> DashboardSnapshot {
    ...
    let position = PositionView {
        ...
        ioc_jump_chase_orders: ioc.jump_chase_orders,
        ioc_jump_chase_filled: ioc.jump_chase_filled,
        ioc_jump_chase_fill_rate: rate(ioc.jump_chase_filled, ioc.jump_chase_orders),
        ioc_last_order: ioc
    ...
    let fair_value = FairValueView {
        ...
        chainlink_price: s.chainlink_price,
        basis_vs_chainlink: s.basis_vs_chainlink,
        binance_jump_1s: s.binance_jump_1s,
        last_jump_age_ms: s.last_jump_ts_ms.map(|ts| now_ms - ts),
        last_jump_dir: s.last_jump_dir,
    };
...
struct IocStats {
    ...
    rebal_orders: usize,
    rebal_filled: usize,
    jump_chase_orders: usize,
    jump_chase_filled: usize,
}
    ...
    for order in orders {
        ...
        match order.reason {
            ...
            PendingOrderReason::JumpChase => {
                stats.jump_chase_orders += 1;
                if filled {
                    stats.jump_chase_filled += 1;
                }
            }
        }
    }
...
fn ioc_order_summary(order: &ManagedOrder) -> String {
    ...
    let reason = match order.reason {
        ...
        PendingOrderReason::JumpChase => "jump",
    };
    let status =
...
/// 重构后的 Order Log 事件生成：
///   - 1 个订单生命周期 → 1 行（不再 POST + FILL + MATCH 三条重复）
///   - kind 4 类：成交 / 拒单 / 挂单 / 撤单
///   - summary 用中文讲清楚"为什么触发、结果是什么"
///   - pnl 列改为本笔现金流（成交=−花费 USDC 或 +收到 USDC；其他=0）
fn order_events(
    trades: &[TradeRecord],
    orders: &[ManagedOrder],
    ...
    // 1) 进行中的挂单：每边一行（最多 2 行）
    for order in [pending_up, pending_down].into_iter().flatten() {
        let reason = reason_name(order.reason).to_string();
    ...
    // 2) 实际成交（来自 trades，包含 maker fill 和 taker fill）
    for trade in trades {
        let liq = if trade.maker_taker { "MAKER" } else { "TAKER" };
        let bs = if trade.buy_sell { "买" } else { "卖" };
        // 现金流：买 → 负（USDC 出），卖 → 正（USDC 入）
        let cash_flow = if trade.buy_sell {
            -(trade.qty * trade.price)
        } else {
            trade.qty * trade.price
        };
        events.push(OrderEventView {
            ...
            reason: String::new(), // trade 没保留 reason 引用，故留空（订单历史那行会补）
            summary: format!(
                "{} {} {:.0}张 @{:.2} = {:+.2} USDC",
                bs,
    ...
    // 3) 订单历史：只保留非成交结局（拒单、撤单等），避免与 trades 的成交行重复
    for order in orders {
        // 若有成交，已由 trades 那一段表达；这里跳过
        if order.filled_qty > 0.0 {
            continue;
        }
        let kind_label = match order.status {
            OrderStatus::Cancelled | OrderStatus::Expired => "撤单",
            OrderStatus::Rejected => "拒单",
            _ if order.reject_reason.as_deref() == Some("worst_breach") => "拒单",
            _ => "结束",
        };
        let reason = reason_name(order.reason).to_string();
        let reject_msg = match order.reject_reason.as_deref() {
            Some("worst_breach") => "盘口比 worst 还贵 — 行情飞了",
            Some("ioc_remainder") => "部分成交后剩余撤掉",
            Some("pair_health_breach") => "pair_health 守门拒绝（risk）",
            Some("max_buy_price") => "价格 > 0.85 上限拒单",
            Some(other) => other,
            None => "未成交",
        };
        events.push(OrderEventView {

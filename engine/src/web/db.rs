...
//!   * `my_orders`     —— POST /order 的完整回执（200/400 都写完整一行；resubmit 链通过 parent_client_order_id 关联）
//!   * `my_fills`      —— user-ws trade 事件 MATCHED 时入库；MINED / CONFIRMED 仅更新 status + tx_hash + last_update
//!
//! 写入路径单线程化：所有写操作通过 mpsc `DbMsg` 投递到 `run_db_writer`，POST 热路径不阻塞 sqlite IO。
...
/// Web UI 每 30s 调用：把当前快照投递到 db writer（异步）。
pub fn persist_sample(db_tx: &DbSender, _state: &AppState, point: &WebPnlPoint) {
    let _ = db_tx.send(DbMsg::PnlSample(point.clone()));
}

...
pub struct AppState {
    ...
    /// --- Chainlink Data Streams（Polymarket 实际结算源）---
    /// 最新 chainlink BTC/USD 价（USD）。Some 即 WS 已建连并有报价；
    /// 这是 K/S/结算价的权威来源，binance 仅作 5s alpha 信号。
    pub chainlink_price: Option<f64>,
    /// chainlink report 的 observationsTimestamp（unix 秒）
    pub chainlink_obs_ts: Option<i64>,
    /// 本机收到报文的 unix 毫秒（用于延迟监控）
    pub chainlink_recv_ts_ms: Option<i64>,
    /// 币安 mid 与 chainlink 的 basis: binance - chainlink；正=binance 高。
    /// jump alpha 触发用：basis 变大意味着 chainlink 还没跟上，5s 内会追。
    pub basis_vs_chainlink: Option<f64>,
    /// --- Binance Jump 触发器（数据证 ±$30/1s 后 chainlink 5s 内 ~126% pass-through）---
    /// 过去 1 秒 binance mid 的最大变化（带符号；正=上涨）
    pub binance_jump_1s: f64,
    /// 上次 ±$30 jump 触发的 unix ms（用于 confirm window 和限频）
    pub last_jump_ts_ms: Option<i64>,
    /// 上次 jump 方向：+1=向上，-1=向下
    pub last_jump_dir: Option<i8>,
    /// --- Deribit BTC 期权 IV smile（独立 σ 源，前瞻性强于 binance realized vol）---
    /// 按 log_moneyness 升序的 (log_mny, iv_decimal) 列表；None = 还没拉到
    pub deribit_surface: Option<Vec<(f64, f64)>>,
    /// ATM (log_mny=0) IV，display 用
    pub deribit_atm_iv: Option<f64>,
    /// 最近一次拉取 Deribit smile 的 unix ms
    pub deribit_refresh_ts_ms: Option<i64>,
}
...
impl AppState {
    pub fn new(symbol: &str) -> Self {
        Self {
            ...
            chainlink_obs_ts: None,
            chainlink_recv_ts_ms: None,
            basis_vs_chainlink: None,
            binance_jump_1s: 0.0,
            last_jump_ts_ms: None,
            last_jump_dir: None,
            deribit_surface: None,
            deribit_atm_iv: None,
            deribit_refresh_ts_ms: None,
        }
    }

...
/// 当前 pending 买单来源。区分追涨建仓、偏仓配平和币安猛跳抢跑，便于独立统计与风控。
/// - Chase: 稳态→激变态切换的 lead-driven 建仓（看 FV vs Poly 长期 gap）
/// - Rebalance: 偏仓时配平对侧，锁 merge 套利
/// - JumpChase: binance 1s ±$30 猛跳抢跑（数据证 chainlink 5s 内跟 ~126%）
#[derive(Clone, Copy, Debug, PartialEq, Eq)]

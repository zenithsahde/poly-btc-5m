...
impl LiveOrderClient {
    pub async fn new(
        ...
        Self::with_http(signer, creds, order_size_usdc, maker_timeout_ms, state, build_http2_client()?).await
    }
...
impl SharedClob {
    async fn post_order(&self, template: reqwest::Request, body: Bytes) -> Result<PlaceOrderResult> {
        ...
        let order_id = json["orderID"].as_str().ok_or_else(|| anyhow!("missing orderID: {text}"))?.to_string();
        text.clear();
        let order_status = json["status"].as_str().unwrap_or("unknown").to_string();
        let filled_size = json["takingAmount"].as_str().and_then(|s| s.parse::<f64>().ok());
...
impl OrderClient for LiveOrderClient {
    async fn place_order(&self, req: PlaceOrderRequest) -> Result<PlaceOrderResult> {
        ...
        self.shared.post_order(clone_template(&self.post_template), body).await
    }

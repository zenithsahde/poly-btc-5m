...
impl PolyWsClient {
    ...
    async fn maybe_switch_market(
        ...
        //   回退：币安「窗口开始这一秒」内的首笔成交价（旧逻辑，~$10 basis 误差但活着）
        //   末路：用当前 mid
        let window_start_ts = new_market.window_end_ts - 300;
        let strike_from_chainlink = match crate::ws::chainlink_ds::ChainlinkConfig::from_env() {
            Ok(cfg) => {
                match crate::ws::chainlink_ds::fetch_report_at(&cfg, window_start_ts).await {
                    Ok(r) => Some(r.price),
                    Err(e) => {
                        tracing::warn!(
                            "Chainlink REST 取 K 失败（{:?}），回退 binance",
                            e
                        );
                        None
                    }
        ...
        if let Ok(mut s) = self.state.write() {
            ...
            s.strike_price = if let Some(k_cl) = strike_from_chainlink {
                info!(
                    ...
                    k_cl, window_start_ts
                );
                k_cl.round()
            } else {
                match strike_from_binance {
                    Ok(open) => {
                        info!(
                            "K 取自币安窗口开始秒内首笔成交（chainlink 未就绪）: {:.2} (ts={})",
